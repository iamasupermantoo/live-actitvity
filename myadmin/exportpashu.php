...
$objPHPExcel->getProperties()->setCreator("某某源码网有限公司")
							 ...
							 ->setTitle("Office 2007 XLSX 微信现场活动系统猴子爬树结果")
							 ->setSubject("Office 2007 XLSX 微信现场活动系统猴子爬树结果")
							 ->setDescription("微信现场活动系统猴子爬树结果.")
							 ->setKeywords("微信现场活动系统猴子爬树结果")
                             ->setCategory("某某源码网有限公司程序导出文件");
$load->model('Pashu_model');
$sheetindex=0;
if($id==0){
    $configs=$load->pashu_model->getAllConfig();
    foreach($configs as $config){
        if($config['status']==3){
            $shake_record=$load->pashu_model->getRecord($config['id']);
    ...
    $shake_record=$load->pashu_model->getRecord($id);
    if(empty($shake_record)){
        return;
    }
