...
require_once(dirname(__FILE__) .DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'wall'.DIRECTORY_SEPARATOR.'biaoqing.php');
...
function setband(){
	...
	if($result){
		...
		if($oldstatus==1 && $status==2){
			$flag_model->decrSignedCount();
		}
		if($oldstatus==2 && $status==1){
			$flag_model->incrSignedCount();
		}
