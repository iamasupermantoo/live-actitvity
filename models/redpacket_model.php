...
class Redpacket_model extends Basemodel{
	...
	public function getRedpacketConfig(){
		...
		$data['sendname']=empty($data['sendname'])?'人人互动':$data['sendname'];
	...
	public function getRoundById($id){
		return $this->_redpacket_round_m->find('id='.$id);
	}
	...
	public function getRoundList(){
		return $this->_redpacket_round_m->select('1 order by id asc');
	}
	//统计红包数量
	...
	public function getRedpacketSendingLog($openid){
		...
		$data=$this->_redpacket_order_return_m->select($where,'mch_billno,total_amount,err_code_des');
		return $data;
	}
	//发放红包
	...
	public function _addRedpacketUser($data){
		return $this->_redpacket_users_m->add($data);
	}
	//初始化红包记录
