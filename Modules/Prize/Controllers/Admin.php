...
class Admin extends Adminbase
{
    ...
    public function ajax_act_save_prize()
    {
        ...
        $prize['rate']=is_numeric($_POST['rate'])?$_POST['rate']*10000:100;
        $prize['type']=1;//isset($_POST['num'])?intval($_POST['num']):0;
        if ($prize['type']==1) {
            $imageid=isset($_POST['imageid'])?intval($_POST['imageid']):0;
            $prize['prizedata']=array(
                'imageid'=>$imageid
            );
            if ($prize['prizedata']['imageid']<=0) {
                if (!empty($_FILES['imagepath']['type'])) {
                    ...
                    $prize['prizedata']['imageid']=$file['id'];
                }
            } else {
    ...
    public function exportdata(){
        ...
        // require_once BASEPATH.DIRECTORY_SEPARATOR.'library'.DIRECTORY_SEPARATOR.'phpexcel'.DIRECTORY_SEPARATOR.'PHPExcel.php';
        $objPHPExcel = new PHPExcel();
        $objPHPExcel->getProperties()->setCreator("某某源码网有限公司")
                                    ->setLastModifiedBy("某某源码网有限公司")
                                    ->setTitle("Office 2007 XLSX 微信现场活动系统中奖结果")
                                    ->setSubject("Office 2007 XLSX 微信现场活动系统中奖结果")
                                    ->setDescription("微信现场活动系统中奖结果.")
                                    ->setKeywords("微信现场活动系统中奖结果")
